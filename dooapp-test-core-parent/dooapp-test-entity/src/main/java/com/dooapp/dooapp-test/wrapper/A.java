...
import com.dooapp.dooapp-test.entity.BBean;
...
// End of user code
{
					...
					while (change.next()) {
						if (change.getAddedSize() > 0) {
							for (B tmp : change.getAddedSubList()) {
								getBean().addB(tmp == null ? null : tmp.getBean());
							}
						}
						if (change.getRemovedSize() > 0) {
							for (B tmp : change.getRemoved()) {
								getBean().getB().remove(tmp == null ? null : tmp.getBean());
							}
						}
					}
				}
	...
	public ObjectProperty<D> dProperty() {
		if (dProperty == null) {
			...
			getBean().setD(getBean().getD() == null ? null : getBean()
			...
			dProperty.addListener(new ChangeListener<D>() {
				...
				public void changed(ObservableValue<? extends D> obj, D oldValue, D newValue) {
					getBean().setD(newValue == null ? null : newValue.getBean());
				}
			});
		}
		return dProperty;
	...
	protected ObjectProperty<D> createDProperty(){
		return new SimpleObjectProperty<D>(getBean().getD() == null ? null : getBean()
							.getD().getWrapper());
	}
	...
	public void setD(D myD){
		if (dProperty == null) {
				getBean().setD(myD == null ? null : myD.getBean());
			} else {
