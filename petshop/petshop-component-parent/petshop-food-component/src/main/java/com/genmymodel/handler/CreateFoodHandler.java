 ...
 * @since 1.0-SNAPSHOT
 */
public class CreateFoodHandler implements EventHandler<LinkEvent<Location>> {
	/**
	 * The logger
	 */
	...
	* The famous {@link EventBus}
	 ...
	 */
	@Inject
	private FoodBuilder itemBuilder;
	/**
	...
	public boolean canHandle(Event event) {
		if (event instanceof LinkEvent) {
			...
			return Action.CREATE.equals(location.getAction()) && ApplicationDomain.FOOD.equals(location.getDomain());
		}
		return false;
	}
	...
	public void handle(LinkEvent<Location> event) {
		Food item = manager.save(itemBuilder.build());
		bus.publish(new LinkEvent<DisplayLocation>(new DisplayLocation(ApplicationDomain.FOOD, Food.class, item.getId())));
	}
}
