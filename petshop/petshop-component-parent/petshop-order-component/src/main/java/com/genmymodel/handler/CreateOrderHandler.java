 ...
 * @since 1.0-SNAPSHOT
 */
public class CreateOrderHandler implements EventHandler<LinkEvent<Location>> {
	/**
	 * The logger
	 */
	...
	* The famous {@link EventBus}
	 ...
	 */
	@Inject
	private OrderBuilder itemBuilder;
	/**
	...
	public boolean canHandle(Event event) {
		if (event instanceof LinkEvent) {
			...
			return Action.CREATE.equals(location.getAction()) && ApplicationDomain.ORDER.equals(location.getDomain());
		}
		return false;
	}
	...
	public void handle(LinkEvent<Location> event) {
		Order item = manager.save(itemBuilder.build());
		bus.publish(new LinkEvent<DisplayLocation>(new DisplayLocation(ApplicationDomain.ORDER, Order.class, item.getId())));
	}
}
