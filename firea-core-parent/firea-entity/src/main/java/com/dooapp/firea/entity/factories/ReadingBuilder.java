...
// End of user code
{
	...
	public Reading build() throws NullPointerException {
		...
		if (point == null) {
			throw new NullPointerException("Reading must be linked to a Point which has not been set.");
		} else { 
	...
	private void handleDefaultDevice(Reading result) {
		result.setDevice(deviceBuilder.build());
	}
	...
	private void handleDefaultGaugemode(Reading result) {
		result.setGaugemode(gaugeModeBuilder.build());
	}
