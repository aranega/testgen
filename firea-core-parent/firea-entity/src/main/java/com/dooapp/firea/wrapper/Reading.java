...
// End of user code
{
	...
	public ObjectProperty<Device> deviceProperty() {
		if (deviceProperty == null) {
			deviceProperty = new SimpleObjectProperty<Device>(reading.getDevice() == null ? null : reading
					.getDevice().getWrapper());
			deviceProperty.addListener(new ChangeListener<Device>() {
				@Override
				public void changed(ObservableValue<? extends Device> obj, Device oldValue, Device newValue) {
					reading.setDevice(newValue == null ? null : newValue.getBean());
				}
			});
	 ...
	 * <!-- End of user code -->
	 */
	public void setDevice(Device myDevice){
		if (deviceProperty == null) {
				reading.setDevice(myDevice == null ? null : myDevice.getBean());
			} else {
				this.deviceProperty().set(myDevice);
			}
	}
	
	public void updateDevice(final Device myDevice, final Object mutex) {
		if (javafx.application.Platform.isFxApplicationThread()) {
			setDevice(myDevice);
			if (mutex != null) {
				mutex.notify();
	...
	public void updateDevice(final Device myDevice) {
		updateDevice(myDevice, null);
	}
	...
	public String toXML(){
		...
		return getReadingBean() != null ? serializer.outputString(getReadingBean().toXML()) : "";
