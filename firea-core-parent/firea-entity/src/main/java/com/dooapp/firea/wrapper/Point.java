...
// End of user code
{
	...
	private ObjectProperty<Reading> readingsProperty;
	 ...
	 * <!-- End of user code -->
	 */
	public ObjectProperty<Reading> readingsProperty() {
		if (readingsProperty == null) {
			readingsProperty = new SimpleObjectProperty<Reading>(point.getReadings() == null ? null : point
					.getReadings().getWrapper());
			readingsProperty.addListener(new ChangeListener<Reading>() {
				@Override
				public void changed(ObservableValue<? extends Reading> obj, Reading oldValue, Reading newValue) {
					point.setReadings(newValue == null ? null : newValue.getBean());
				}
			});
	...
	/*
	 * <!-- Start of user code comment for readings setter 
		-->
	 * <!-- End of user code -->
	 */
	public void setReadings(Reading myReadings){
		if (readingsProperty == null) {
				point.setReadings(myReadings == null ? null : myReadings.getBean());
			} else {
				this.readingsProperty().set(myReadings);
			}
	}
	
	public void updateReadings(final Reading myReadings, final Object mutex) {
		if (javafx.application.Platform.isFxApplicationThread()) {
	...
	public void updateReadings(final Reading myReadings) {
		updateReadings(myReadings, null);
	}
	...
	public String toXML(){
		...
		return getPointBean() != null ? serializer.outputString(getPointBean().toXML()) : "";
