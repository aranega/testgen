 ...
 * @generated
 */
public class CreateProjectHandler implements EventHandler<LinkEvent<Location>> {
	/**
	 ...
	 */
	@Inject
	private ProjectBuilder itemBuilder;
	/**
	 ...
	 */
	@Inject
	protected CreateProjectHandler() {
	}
	...
	public boolean canHandle(Event event) {
		if (event instanceof LinkEvent) {
			...
			return Action.CREATE.equals(location.getAction()) && ApplicationDomain.PROJECT.equals(location.getDomain());
		}
		return false;
	}
	...
	public void handle(LinkEvent<Location> event) {
		Project item = manager.save(itemBuilder.build());
		bus.publish(new LinkEvent<DisplayLocation>(new DisplayLocation(ApplicationDomain.PROJECT, Project.class, item.getId())));
	}
}
