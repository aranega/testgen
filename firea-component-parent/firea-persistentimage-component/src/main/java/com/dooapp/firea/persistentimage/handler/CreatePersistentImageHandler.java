...
/**
 * Handler to create PersistentImage<br>
 * <br>
 * Created at : date<br>
 * 
 * @author genmymodel
 ...
 * @generated
 */
public class CreatePersistentImageHandler implements EventHandler<LinkEvent<Location>> {
	/**
     ...
     * @generated
	 */
	private static final Logger logger = LoggerFactory.getLogger(CreatePersistentImageHandler.class);
	 ...
	 */
	@Inject
	private PersistentImageBuilder itemBuilder;
	/**
	 ...
	 */
	@Inject
	protected CreatePersistentImageHandler() {
	}
	...
	public boolean canHandle(Event event) {
		if (event instanceof LinkEvent) {
			...
			return Action.CREATE.equals(location.getAction()) && ApplicationDomain.PERSISTENTIMAGE.equals(location.getDomain());
		}
		return false;
	}
	...
	public void handle(LinkEvent<Location> event) {
		PersistentImage item = manager.save(itemBuilder.build());
		bus.publish(new LinkEvent<DisplayLocation>(new DisplayLocation(ApplicationDomain.PERSISTENTIMAGE, PersistentImage.class, item.getId())));
	}
}
