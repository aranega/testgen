...
/**
 * Handler to create TestedRoom<br>
 * <br>
 * Created at : date<br>
 * 
 * @author genmymodel
 ...
 * @generated
 */
public class CreateTestedRoomHandler implements EventHandler<LinkEvent<Location>> {
	/**
     ...
     * @generated
	 */
	private static final Logger logger = LoggerFactory.getLogger(CreateTestedRoomHandler.class);
	 ...
	 */
	@Inject
	private TestedRoomBuilder itemBuilder;
	/**
	 ...
	 */
	@Inject
	protected CreateTestedRoomHandler() {
	}
	...
	public boolean canHandle(Event event) {
		if (event instanceof LinkEvent) {
			...
			return Action.CREATE.equals(location.getAction()) && ApplicationDomain.TESTEDROOM.equals(location.getDomain());
		}
		return false;
	}
	...
	public void handle(LinkEvent<Location> event) {
		TestedRoom item = manager.save(itemBuilder.build());
		bus.publish(new LinkEvent<DisplayLocation>(new DisplayLocation(ApplicationDomain.TESTEDROOM, TestedRoom.class, item.getId())));
	}
}
