...
/**
 * Handler to create CalibrationStep<br>
 * <br>
 * Created at : date<br>
 * 
 * @author genmymodel
 ...
 * @generated
 */
public class CreateCalibrationStepHandler implements EventHandler<LinkEvent<Location>> {
	/**
     ...
     * @generated
	 */
	private static final Logger logger = LoggerFactory.getLogger(CreateCalibrationStepHandler.class);
	 ...
	 */
	@Inject
	private CalibrationStepBuilder itemBuilder;
	/**
	 ...
	 */
	@Inject
	protected CreateCalibrationStepHandler() {
	}
	...
	public boolean canHandle(Event event) {
		if (event instanceof LinkEvent) {
			...
			return Action.CREATE.equals(location.getAction()) && ApplicationDomain.CALIBRATIONSTEP.equals(location.getDomain());
		}
		return false;
	}
	...
	public void handle(LinkEvent<Location> event) {
		CalibrationStep item = manager.save(itemBuilder.build());
		bus.publish(new LinkEvent<DisplayLocation>(new DisplayLocation(ApplicationDomain.CALIBRATIONSTEP, CalibrationStep.class, item.getId())));
	}
}
