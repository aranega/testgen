 ...
 * @generated
 */
public class CreateFolderHandler implements EventHandler<LinkEvent<Location>> {
	/**
     ...
     * @generated
	 */
	private static final Logger logger = LoggerFactory.getLogger(CreateFolderHandler.class);
	 ...
	 */
	@Inject
	private FolderBuilder itemBuilder;
	/**
	...
	public boolean canHandle(Event event) {
		if (event instanceof LinkEvent) {
			...
			return Action.CREATE.equals(location.getAction()) && ApplicationDomain.FOLDER.equals(location.getDomain());
		}
		return false;
	}
	...
	public void handle(LinkEvent<Location> event) {
		Folder item = manager.save(itemBuilder.build());
		bus.publish(new LinkEvent<DisplayLocation>(new DisplayLocation(ApplicationDomain.FOLDER, Folder.class, item.getId())));
	}
}
