 ...
 * @generated
 */
public class CreateReadingHandler implements EventHandler<LinkEvent<Location>> {
	/**
     ...
     * @generated
	 */
	private static final Logger logger = LoggerFactory.getLogger(CreateReadingHandler.class);
	 ...
	 */
	@Inject
	private ReadingBuilder itemBuilder;
	/**
	 ...
	 */
	@Inject
	protected CreateReadingHandler() {
	}
	...
	public boolean canHandle(Event event) {
		if (event instanceof LinkEvent) {
			...
			return Action.CREATE.equals(location.getAction()) && ApplicationDomain.READING.equals(location.getDomain());
		}
		return false;
	}
	...
	public void handle(LinkEvent<Location> event) {
		Reading item = manager.save(itemBuilder.build());
		bus.publish(new LinkEvent<DisplayLocation>(new DisplayLocation(ApplicationDomain.READING, Reading.class, item.getId())));
	}
}
