 ...
 * @generated
 */
public class CreateContactHandler implements EventHandler<LinkEvent<Location>> {
	/**
	 ...
	 */
	@Inject
	private ContactBuilder itemBuilder;
	/**
	...
	public boolean canHandle(Event event) {
		if (event instanceof LinkEvent) {
			...
			return Action.CREATE.equals(location.getAction()) && ApplicationDomain.CONTACT.equals(location.getDomain());
		}
		return false;
	}
	...
	public void handle(LinkEvent<Location> event) {
		Contact item = manager.save(itemBuilder.build());
		bus.publish(new LinkEvent<DisplayLocation>(new DisplayLocation(ApplicationDomain.CONTACT, Contact.class, item.getId())));
	}
}
