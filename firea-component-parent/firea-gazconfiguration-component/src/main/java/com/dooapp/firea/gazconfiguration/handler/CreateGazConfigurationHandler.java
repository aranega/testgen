...
/**
 * Handler to create GazConfiguration<br>
 * <br>
 * Created at : date<br>
 * 
 * @author genmymodel
 ...
 * @generated
 */
public class CreateGazConfigurationHandler implements EventHandler<LinkEvent<Location>> {
	/**
     ...
     * @generated
	 */
	private static final Logger logger = LoggerFactory.getLogger(CreateGazConfigurationHandler.class);
	 ...
	 */
	@Inject
	private GazConfigurationBuilder itemBuilder;
	/**
	 ...
	 */
	@Inject
	protected CreateGazConfigurationHandler() {
	}
	...
	public boolean canHandle(Event event) {
		if (event instanceof LinkEvent) {
			...
			return Action.CREATE.equals(location.getAction()) && ApplicationDomain.GAZCONFIGURATION.equals(location.getDomain());
		}
		return false;
	}
	...
	public void handle(LinkEvent<Location> event) {
		GazConfiguration item = manager.save(itemBuilder.build());
		bus.publish(new LinkEvent<DisplayLocation>(new DisplayLocation(ApplicationDomain.GAZCONFIGURATION, GazConfiguration.class, item.getId())));
	}
}
