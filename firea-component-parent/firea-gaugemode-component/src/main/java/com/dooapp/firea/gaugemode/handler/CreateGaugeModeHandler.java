...
/**
 * Handler to create GaugeMode<br>
 * <br>
 * Created at : date<br>
 * 
 * @author genmymodel
 ...
 * @generated
 */
public class CreateGaugeModeHandler implements EventHandler<LinkEvent<Location>> {
	/**
     ...
     * @generated
	 */
	private static final Logger logger = LoggerFactory.getLogger(CreateGaugeModeHandler.class);
	 ...
	 */
	@Inject
	private GaugeModeBuilder itemBuilder;
	/**
	 ...
	 */
	@Inject
	protected CreateGaugeModeHandler() {
	}
	...
	public boolean canHandle(Event event) {
		if (event instanceof LinkEvent) {
			...
			return Action.CREATE.equals(location.getAction()) && ApplicationDomain.GAUGEMODE.equals(location.getDomain());
		}
		return false;
	}
	...
	public void handle(LinkEvent<Location> event) {
		GaugeMode item = manager.save(itemBuilder.build());
		bus.publish(new LinkEvent<DisplayLocation>(new DisplayLocation(ApplicationDomain.GAUGEMODE, GaugeMode.class, item.getId())));
	}
}
