 ...
 * @generated
 */
public class CreatePointHandler implements EventHandler<LinkEvent<Location>> {
	/**
     ...
     * @generated
	 */
	private static final Logger logger = LoggerFactory.getLogger(CreatePointHandler.class);
	 ...
	 */
	@Inject
	private PointBuilder itemBuilder;
	/**
	...
	public boolean canHandle(Event event) {
		if (event instanceof LinkEvent) {
			...
			return Action.CREATE.equals(location.getAction()) && ApplicationDomain.POINT.equals(location.getDomain());
		}
		return false;
	}
	...
	public void handle(LinkEvent<Location> event) {
		Point item = manager.save(itemBuilder.build());
		bus.publish(new LinkEvent<DisplayLocation>(new DisplayLocation(ApplicationDomain.POINT, Point.class, item.getId())));
	}
}
