...
// End of user code
{
	...
	public Order build() throws NullPointerException {
		...
		if (customer == null) {
			throw new NullPointerException("Order must be contained by a Customer which has not been set.");
		} else { 
	...
	private void handleDefaultStatus(Order result) {
		...
		result.setStatus(cfg.getProperty("defaultValue.Order.status"));
		// End of user code
	}
